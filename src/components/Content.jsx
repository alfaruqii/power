  ...
  return (
    <>
      <motion.div
        ...
        <h2 className="font-bold mb-1 text-lg lg:text-2xl">{locate[0][Object.keys(locate[0])]}</h2>
        <div className="grid grid-cols gap-5 indent-5 lg:text-lg">
          {
            locate[1].map((par, i) => {
              if (Object.keys(par).includes("pic")) {
                const imgName = par[Object.keys(par)].match(/[A-Za-z0-9]\w+/)[0].replace(/([A-Z])/g, ' $1').trim()
                return <div key={i} className="my-5 text-center  sm:max-h-[28rem] flex flex-col items-center justify-center">
                  <img src={par[Object.keys(par)]} alt={imgName} className="mb-2 shadow-lg sm:h-full" />
          ...
          locate[2] ?
            <div className="mt-8">
              ...
              <div className="grid grid-cols gap-3 indent-5 lg:text-lg">
                ...
                {
                  locate[3] ? <div className="text-center italic">
                    {
                      locate[3].map((part, i) => {
